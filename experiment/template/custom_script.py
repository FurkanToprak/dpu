...
TEMP_INITIAL = [30] * tubeCount

STIR_INITIAL = [8] * tubeCount #try 8,10,12 etc; makes 16-value list
...
def turbidostat(eVOLVER, input_data, vials, elapsed_time, settings):
    ...
    OD_values_to_average = options.to_avg  # Number of values to calculate the OD average
    EXP_NAME = options.exp_name # Name of experiment files

    lower_thresh = [options.lower_threshold] * len(vials) #to set all vials to the same value, creates 16-value list
    upper_thresh = [options.upper_threshold] * len(vials) #to set all vials to the same value, creates 16-value list

    ##### Turbidostat Settings #####
    # Tunable settings for overflow protection, pump scheduling etc. Unlikely to change between expts

    time_out = 5 #(sec) additional amount of time to run efflux pump TODO: Check if needed
    pump_wait = 3 # (min) minimum amount of time to wait between pump events TODO: Check if needed
        ...
        if data.size != 0:
            ...
            if average_OD > ODset and collecting_more_curves:
                ...
                if ((elapsed_time - last_pump)*60) >= pump_wait: # if sufficient time since last pump, send command to Arduino
                    ...
                    # efflux pump
                    MESSAGE[x + 16] = 0 # str(time_in + time_out)
...
def chemostat(eVOLVER, input_data, vials, elapsed_time, settings):
